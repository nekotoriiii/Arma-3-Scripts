// grab dropped items
_crate = createVehicle ["I_supplyCrate_F", player getPos [2, getDir player]];
...
_supplies = ((getPos player) nearSupplies 2000) select { 
    (_x isKindOf "Man" && !(alive _x)) ||  
    {(typeOf _x) in (["WeaponHolderSimulated", "GroundWeaponHolder", "WeaponHolder"])} 
};
